...
for (const marker of ['data-footnotes', 'data-footnote-ref', '각주 1의 본문으로 돌아가기', '>각주</h2>', 'class="markdown-figure"', '<figcaption>촬영 당시 확인한 목성의 대기 띠</figcaption>', 'loading="lazy"', 'class="table-scroll"', 'scope="col"', '좌우로 스크롤할 수 있습니다']) {
  if (!code.includes(marker)) throw new Error(`Markdown 각주 렌더링 검사 실패: ${marker}`);
}

console.log('Markdown 기능 검사: 각주·이미지 캡션·반응형 표 렌더링 이상 없음');
